...
> Portable macro recording for [RoRoRo](https://github.com/estevanhernandez-stack-ed/ROROROblox)-managed Roblox alts. Record once, play on any alt — with window-relative mouse macros (clicks land right at any window position, on any monitor), round-robin assignments + AFK keep-alive, multi-select sequential playback, one-click STACK/GRID window arranging, multi-window mode, compact view for long sequences, and an action bridge so sibling plugins (like [RoRoRo Ur OCR](https://github.com/estevanhernandez-stack-ed/Ur-OCR)) can fire macros on screen triggers.
